...
def _coo_at_plant(mat, plant):
    """Country of origin maintained at the PLANT view (A_ProductPlant.CountryOfOrigin = MARC-HERKL),
    which is the canonical home for a material's CoO. The A_Product HEADER carries a CountryOfOrigin
    too but it's frequently blank even when the plant view is filled -- so reading only the header
    raised a FALSE 'not specified'. Returns the code, or None (blank / read failed). Probed 2026-06-16:
    the PIR (A_PurchasingInfoRecord in API_INFORECORD_PROCESS_SRV) exposes NO origin field on this
    surface, so -- ground, don't guess -- we do NOT read CoO from the PIR."""
    try:
        raw = explore_entity("A_ProductPlant", filter=f"Product eq '{mat}' and Plant eq '{plant}'",
                             service="API_PRODUCT_SRV")
        if not isinstance(raw, str) or raw.startswith("SAP request failed"):
            return None
        d = json.loads(raw).get("d", {})
        rows = d.get("results", [d]) if isinstance(d, dict) else []
        v = str((rows[0].get("CountryOfOrigin") if rows else "") or "").strip().upper()
        return v or None
    except (json.JSONDecodeError, ValueError, TypeError, KeyError):
        return None


def _effective_coo(obj, plant):
    """The material's EFFECTIVE country of origin and where it came from: the header value if set, else
    the plant-view value. Grounds check_country_of_origin against where CoO actually lives (B5) instead
    of the header alone. Returns (code_or_'', source)."""
    hdr = str(obj.get("CountryOfOrigin") or "").strip().upper()
    if hdr:
        return hdr, "header"
    mat = obj.get("Product")
    if mat and plant:
        at_plant = _coo_at_plant(mat, plant)
        if at_plant:
            return at_plant, f"plant {plant}"
...
def assure_assembly(material: str, plant: str = "1710", components: list | None = None) -> str:
    ...
    if parent:
        coo, coo_src = _effective_coo(parent, plant)   # B5: read CoO where it lives (header -> plant view)
        parent["CountryOfOrigin"] = coo
        facts["parent"] = {k: parent.get(k) for k in FACT_FIELDS}
        facts["parent"]["CountryOfOriginSource"] = coo_src
    ...
    for c in comps:
        ...
        coo, coo_src = _effective_coo(o, plant)   # B5: CoO from where it lives, not the (often blank) header
        o["CountryOfOrigin"] = coo
        ...
        comp_facts.append({"Product": o.get("Product"), "description": _desc(c),
                           ...
                           "CountryOfOrigin": coo, "CountryOfOriginSource": coo_src})
