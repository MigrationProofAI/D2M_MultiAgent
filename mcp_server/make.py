...
def _require_matnr(material: str) -> str | None:
    """A BOM is keyed by the material NUMBER, not its name. Models sometimes pass the description
    (e.g. "ASUS ROG Gaming Laptop"), which SAP rejects as "material number is longer than the length
    set". Catch that here and steer the model to resolve the number first. Returns an error string to
    return-early, or None if the key looks like a real material number."""
    m = (material or "").strip()
    if " " in m or len(m) > 18:
        return (f"ERROR: '{material}' looks like a material NAME, not a material NUMBER. A BOM is keyed "
                f"by the number (e.g. 11330). Call search_materials(description='{m}') to get the number, "
                "then retry with that number as `material`.")
    return None
