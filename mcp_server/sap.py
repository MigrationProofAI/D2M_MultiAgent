...
def create_material(fields: dict, confirm: bool = False) -> str:
    ...
    """
    # Tolerate a model passing a stray top-level description: A_Product has NO 'description' property
    # (it's ProductDescription inside to_Description), so SAP rejects it as "the property 'description'
    # is invalid". Move it into to_Description so the create succeeds.
    fields = dict(fields or {})
    _desc = None
    for _k in ("description", "Description", "ProductDescription"):
        _v = fields.pop(_k, None)
        if isinstance(_v, str) and _v and not _desc:
            _desc = _v
    if _desc and "to_Description" not in fields:
        fields["to_Description"] = {"results": [{"Language": "EN", "ProductDescription": _desc[:40]}]}
    # B1: coded-field grounding backstop -- don't let an ungrounded coded field reach SAP and bounce
    # back as a cryptic reject (M3/180, MG/172). policies.json -> coded_grounding controls enforcement.
    from grounding import enforce as _enforce_grounding
    _gblock = _enforce_grounding(fields, require=True)
    if _gblock and confirm:
        return _gblock
    url = f"{_PRODUCT_SRV}/A_Product"
    if not confirm:
        heads_up = ("\n\n" + _gblock) if _gblock else ""
        return ("PREVIEW — nothing written. Confirm with the user, then call again "
                f"with confirm=true.\nPOST {url}\n"
                f"{json.dumps(fields, indent=2, ensure_ascii=False)}" + heads_up)
...
# Extend per system; default falls back to USD. 1010/1020 = Germany (EUR), 1710/1720 = US (USD).
_PLANT_CURRENCY = {"1010": "EUR", "1020": "EUR", "1710": "USD", "1720": "USD"}


@mcp.tool()
def extend_to_plant(material: str, plant: str, product_type: str = "ROH",
                    mrp_type: str = "ND", procurement_type: str | None = None,
                    valuation_class: str | None = None, standard_price: float = 100.0,
                    currency: str | None = None, confirm: bool = False) -> str:
    """Extend an EXISTING material to a new plant the RIGHT way: POST new child rows to
    A_ProductPlant (flat MARC view) + A_ProductValuation. Use THIS for a plant extension --
    NOT create_material (which POSTs a brand-new product and fails/duplicates).

    Encodes the plant-extension RELATIONS (each learned from a real SAP rejection):
      * plant view is FLAT -- a nested supply-planning deep insert is rejected here
        (API_PRD_MSG/003 "multiple products in one change set").
      * MRP type PD forces a lot size that is NOT an A_ProductPlant property, so the default
        is ND (no planning) for a clean extension; set up MRP afterwards via the supply-
        planning view if the plant needs it.
      * valuation currency is the valuation AREA's LOCAL currency (plant 1010 = EUR, 1710 =
        USD), NOT the material's original currency -- USD into 1010 is rejected (PMD_MSG/029).
      * valuation class defaults by material type (HAWA 3100, FERT 7920, HALB 7900, ROH 3000).

    Storage location is plant-SPECIFIC master data and is NOT set here -- check valid storage
    locations for the plant directly in SAP, then add to_StorageLocation separately if needed.

    SAFETY GATE: confirm=false (default) PREVIEWS both POSTs; confirm=true writes.
    """
    proc = procurement_type or ("E" if product_type in _MADE_TYPES else "F")
    cur = currency or _PLANT_CURRENCY.get(plant, "USD")
    vclass = valuation_class or _VALUATION_CLASS_BY_TYPE.get(product_type, "3000")
    plant_fields = {"PurchasingGroup": "002", "ProfitCenter": "YB700", "MRPType": mrp_type,
                    "ProcurementType": proc, "AvailabilityCheckType": "02"}
    val_fields = {"ValuationType": "", "ValuationClass": vclass,
                  "InventoryValuationProcedure": "V", "PriceDeterminationControl": "2",
                  "StandardPrice": str(standard_price), "PriceUnitQty": "1", "Currency": cur}
    # B1: ground the coded + plant-specific fields before writing (extension is a partial view set,
    # so require=False -- ProductType/BaseUnit/ProductGroup live on the existing material, not here).
    from grounding import enforce as _enforce_grounding
    _gblock = _enforce_grounding({**plant_fields, **val_fields, "Plant": plant}, plant=plant, require=False)
    if _gblock and confirm:
        return _gblock
    p = change_material_view("A_ProductPlant", {"Product": material, "Plant": plant},
                             plant_fields, "add", confirm=confirm)
    v = change_material_view("A_ProductValuation", {"Product": material, "ValuationArea": plant},
                             val_fields, "add", confirm=confirm)
    return f"--- PLANT VIEW (A_ProductPlant) ---\n{p}\n\n--- VALUATION VIEW (A_ProductValuation) ---\n{v}"
...
_FIELD_ALIASES = {
    # human labels the model uses -> the exact OData field; without this the agent reports
    # "codebook not available" when it passes 'material type'/'base unit'/'product group'.
    "material type": "ProductType", "materialtype": "ProductType", "type": "ProductType",
    "base unit": "BaseUnit", "baseunit": "BaseUnit", "unit": "BaseUnit", "uom": "BaseUnit",
    "unit of measure": "BaseUnit",
    "product group": "ProductGroup", "material group": "ProductGroup", "group": "ProductGroup",
    "industry": "IndustrySector", "industry sector": "IndustrySector", "sector": "IndustrySector",
    "status": "CrossPlantStatus", "cross plant status": "CrossPlantStatus", "material status": "CrossPlantStatus",
    # org / plant / MRP / valuation / sales -- the fields the agent kept "not finding"
    "plant": "Plant", "plants": "Plant",
    "mrp type": "MRPType", "mrp controller": "MRPController",
    "procurement type": "ProcurementType", "special procurement": "SpecialProcurementType",
    "purchasing group": "PurchasingGroup", "division": "Division",
    "valuation class": "ValuationClass", "price control": "PriceControl",
    "availability check": "AvailabilityCheckType", "plant status": "PlantSpecificMaterialStatus",
    "item category group": "ItemCategoryGroup", "distribution channel": "ProductDistributionChnl",
    "sales status": "ProductSalesStatus", "storage condition": "WarehouseStorageCondition",
    "temperature condition": "TemperatureConditionInd",
    # B3: classic SAP field names -- incl. the classic Material* -> OData Product* rename. The model
    # naturally reaches for these; without the alias, e.g. MaterialGroup misses and the agent loops.
    "materialgroup": "ProductGroup", "matgrp": "ProductGroup", "material grp": "ProductGroup",
    "materialtype": "ProductType", "mattype": "ProductType",
    "valuationclass": "ValuationClass", "procurementtype": "ProcurementType",
    "industrysector": "IndustrySector", "availabilitycheck": "AvailabilityCheckType",
}
...
def list_allowed_values(field: str) -> str:
    ...
    raw = (field or "").strip()
    field = _FIELD_ALIASES.get(raw.lower(), raw)               # tolerate human labels
    by_odata = book.get("by_odata_field", {})
    by_db = book.get("by_db_field", {})

    def _norm(s):                                              # case / underscore / space-insensitive
        return (s or "").lower().replace("_", "").replace(" ", "")
    target = _norm(field)

    # resolve in order: OData name -> a bridged DB field (e.g. 'WERKS' -> Plant) -> raw DB catalog
    key = next((k for k in by_odata if _norm(k) == target), None)
    entry = by_odata.get(key) if key else None
    if entry is None:
        key = next((k for k, e in by_odata.items() if _norm(e.get("db_field", "")) == target), None)
        entry = by_odata.get(key) if key else None
    if entry is None:
        dbkey = next((k for k in by_db if _norm(k) == target), None)
        if dbkey:
            key, entry = dbkey, by_db[dbkey]
    if entry is None:
        names = ", ".join(sorted(by_odata)) or "(none)"
        return (f"'{raw}' is not a coded field in the code book.\n"
                f"Agent-ready (OData) fields: {names}.\n"
                f"The full catalog also has {len(by_db)} DB-named coded fields (query by the DB "
                f"name, e.g. EXTWG). For plant codes use list_plants; storage locations are "
                f"plant-specific master data, not in the code book.")
    values = entry.get("values", [])
    src = entry.get("check_table") or (f"domain {entry.get('domain')}" if entry.get("domain") else "")
    if not values:
        return f"'{key}'{f' ({src})' if src else ''} has no values in the code book."
    head = f"Allowed values for {key}" + (f" ({src})" if src else "")
    if entry.get("keyed_by"):                                  # the relation: what scopes this list
        head += f" — scoped by {', '.join(entry['keyed_by'])}"
    lines = [head + ":"]
